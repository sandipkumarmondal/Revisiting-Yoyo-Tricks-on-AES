...
/*** 5-ronud encryption of AES ***/

void enc(u8 state[16], u8 subkey[11][16]){   
...
/*** 5-ronud decryption of AES ***/
void dec(u8 state[16], u8 subkey[11][16]){ 
		
		int i;
		for(i = 5; i >= 1; i--){
			invKeyAddition(state,subkey[i]);
			invMixColumns(state);
			invShiftRows(state);
			invSubBytes(state);	
			}	
